...
export const ResultView: React.FC<ResultViewProps> = ({ result, exams, teachers, rooms, mode }) => {
    ...
    <div className="space-y-8 animate-fade-in w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow border border-slate-200">
            ...
            <div className="text-2xl font-bold text-primary">{(avgLoad / 60).toFixed(1)} 小时</div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow border border-slate-200">
            ...
            <tbody>
              {rooms.map((room, idx) => (
                <tr key={room.id} className="hover:bg-slate-50">
                  <td className="border border-slate-300 p-2 text-slate-500">高三</td>
              ...
              <tr className="bg-blue-50 border-t-2 border-slate-400">
                <td className="border border-slate-300 p-2 font-bold text-slate-700" colSpan={3}>楼道</td>
                {tableStructure.map(day => 
                  day.sessions.map(session => 
                    session.exams.map(exam => {
